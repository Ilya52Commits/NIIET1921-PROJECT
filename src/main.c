...
// -- Extern variables | Внешние переменные ---------------------------------------------------------
// exterm - означает, что она является внешней переменной
// Внешняя переменная - это переменная, которая объявлена в одном файле программы и может быть использована в других файлах программы
extern uint16_t mb_holding_regs[];			// 
extern uint16_t	mb_input_regs[];			// регистр ввода
extern unsigned char mb_discretes[];		//
extern unsigned char mb_coils[];			//

extern uint8_t copy[2];				// массив копирования
extern uint8_t user_data[];			// массив данных пользователя

extern uint8_t p1_sample_volume;								//
extern uint8_t t1_sample_volume;								// 
extern uint16_t out1_N, out1_Nmin, out1_Nmax;					// 
extern float out1_Umin, out1_Umax, tmp;							// описание максимального и минимального напряжения

extern float p_out1_max, p_out1_min;							//

//-- Peripheral init function prototypes | Прототипы функций инициализации периферийных устройств ---------------------------------------
...
//-- Main ----------------------------------------------------------------------
// Префикс __attribute__ ((noreturn)) перед объявлением функции main указывает на то, 
// что функция main не возвращает управление в вызывающую программу
// Это означает, что после выполнения функции main программа не будет продолжать свое выполнение, а завершится
__attribute__((noreturn)) int main(void)
{
	// Инициализация переменной i
	int i;

	// Изменение системной частоты на PLLClk
	RCU_SysClkChangeCmd(RCU_SysClk_PLLClk);

	// Автоматическая настройка PLL с заданной частотой
	...
	// Порты GPIO используются для взаимодействия с внешними устройствами, такими как датчики, светодиоды, кнопки и другие периферийные устройства
	...
	// Установка версии прошивки в регистры Modbus
	mb_input_regs[FW_ID_L] = 0x0001;
	mb_input_regs[FW_ID_H] = 0x0100;

	// Проверка и установка скорости передачи данных и адреса Modbus
	...
	// Получение и сохранение идентификаторов чипа и процессора в регистры Modbus
	...
	while (1)
	{
		// Обработка Modbus на каждом такте системного таймера
		...
		// Проверка наличия запроса на обновление прошивки и выполнение соответствующих действий
		if (command_status & FW_UPDATE_MODE)
		{
			// Проверка пароля для обновления прошивки
		...
		// Обновление пользовательских данных из Modbus и выполнение соответствующих действий
		...
		// Выполнение автоматической адресации, если требуется
